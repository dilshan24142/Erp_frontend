...
import { Wallet, Plus, Search, Filter, Pencil, Trash2 } from 'lucide-react';
...
export function AccountsReceivable() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<AccountsReceivable | null>(null);
  const [customers, setCustomers] = useState<any[]>([]);

  useEffect(() => {
    import('@/services/customerService')
      .then(m => m.default.getAll({ size: 500 }).then(res => setCustomers(res.content ?? [])))
      .catch(console.error);
  }, []);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await accountsReceivableService.getAll({ size: 100 });
 ...
 const handleSubmit = async () => {
  ...
  const handleDelete = async () => {
    ...
    await accountsReceivableService.delete(deleteConfirm.id);
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            <Wallet className="w-8 h-8 text-blue-600" /> Accounts Receivable
          </h1>
          <p className="text-gray-500 mt-1">Manage receivables from customers</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Receivable', val: fmt(total), color: 'text-gray-900' },
          { label: 'Amount Received', val: fmt(paid), color: 'text-green-600' },
          { label: 'Outstanding', val: fmt(total - paid), color: 'text-orange-600' },
        ].map(s => (
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
        <div className="flex gap-4">
          <div className="flex-1 relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search customer or invoice..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        ...
        {loading ? <div className="p-8 text-center text-gray-400">Loading data...</div> : (
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Customer', 'Invoice No.', 'Due Date', 'Total', 'Received', 'Outstanding', 'Status', 'Actions'].map(h => <th key={h} className="text-left px-4 py-3 text-sm font-semibold text-gray-900">{h}</th>)}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="px-4 py-4 text-sm font-medium text-gray-900">{r.customer?.name ?? '-'}</td>
      ...
      <Modal open={modal === 'view'} onClose={close} title="Receivable Details" size="md" footer={<ModalBtn onClick={close}>Close</ModalBtn>}>
        {selected && (
          <div>
            ...
            <DetailRow label="Received" value={fmt(selected.paidAmount)} />
      ...
      <Modal
        open={modal === 'form'}
        onClose={close}
        title={selected ? 'Edit Invoice' : 'New Invoice'}
        size="md"
      ...
      <Modal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Confirm Delete" size="sm"
        ...
        <p>Are you sure you want to delete invoice <strong>{deleteConfirm?.invoiceNumber}</strong>?</p>
      </Modal>
    </div>
  );
}