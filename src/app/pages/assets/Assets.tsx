...
export function Assets() {
  ...
  const handleSubmit = () => {
    if (selected) {
      // Update
      assetService.update(selected.id, { ...form, id: selected.id })
        .then(() => { load(); close(); })
        .catch(console.error);
    } else {
      // Create
      assetService.create(form)
        .then(() => { load(); close(); })
        .catch(console.error);
    }
  };
        ...
        {loading ? (
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Code', 'Name', 'Category', 'Location', 'Purchase Date', 'Purchase Price', 'Status', 'Actions'].map(h => (
